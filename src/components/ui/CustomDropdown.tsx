  ...
  return (
    <div ref={dropdownRef} className={`relative w-full ${className}`}>
      ...
      <button
        ...
        className={`w-full px-4 py-3 rounded-xl bg-[#faf9fc] hover:bg-white text-sm flex items-center justify-between text-start transition-all duration-200 border cursor-pointer ${
          hasError
            ...
            : isOpen
            ? 'border-pink-500 ring-2 ring-pink-500/20 bg-white'
      ...
      {isOpen && (
        <div
          role="listbox"
          className="absolute top-full mt-1.5 inset-x-0 z-40 bg-white/95 backdrop-blur-2xl rounded-2xl p-1.5 shadow-xl border border-pink-100/90 space-y-1 animate-scale-up max-h-60 overflow-y-auto"
        >
          {options.map((option) => {
            ...
            return (
              <div
                ...
                className={`px-3.5 py-2.5 rounded-xl text-xs sm:text-sm font-medium flex items-center justify-between transition-all duration-150 cursor-pointer ${
                  isSelected
                    ? 'bg-[#131d38] text-white font-bold shadow-xs'
                    : 'text-slate-700 hover:bg-pink-50 hover:text-pink-700'
                }`}
              >
                <span className="truncate">{option.label}</span>
                {isSelected && <Check size={14} className="text-pink-400 shrink-0" />}
              </div>
            );
          })}
        </div>
